...
const FlashDeals = () => {
  ...
  return (
    <section className="py-5 mobile-xs:py-6 mobile-l:py-8 sm:py-12 tablet:py-16 lg:py-16 bg-light-gray">
      <div className="container mx-auto px-4 mobile-l:px-6 sm:px-8 tablet:px-10">
        <div className="flex flex-col mobile-l:flex-row items-start mobile-l:items-center justify-between mb-4 mobile-l:mb-6 sm:mb-10 tablet:mb-12 lg:mb-12 gap-3 mobile-l:gap-4 sm:gap-5 tablet:gap-6">
          <div className="flex flex-col mobile-l:flex-row items-start mobile-l:items-center gap-2 mobile-l:gap-3 sm:gap-5 tablet:gap-6">
            <h2 className="text-lg mobile-l:text-xl sm:text-2xl tablet:text-3xl lg:text-3xl font-bold text-charcoal">FLASH SALE</h2>
            <div className="bg-destructive/10 border border-destructive/20 rounded-lg px-2.5 mobile-l:px-3 sm:px-4 tablet:px-5 py-1 mobile-l:py-1.5 sm:py-2 tablet:py-2 shadow-lg">
              <div className="flex items-center gap-1.5 mobile-l:gap-2 sm:gap-2 tablet:gap-2">
                <Clock className="w-3 h-3 mobile-l:w-3 mobile-l:h-3 sm:w-4 sm:h-4 tablet:w-4 tablet:h-4 text-destructive" />
                <span className="text-xs mobile-l:text-xs sm:text-sm tablet:text-base font-medium text-destructive">Ends in {formattedTime}</span>
              </div>
            </div>
          </div>
          <Link to="/flash-deals">
        ...
        <div className="grid mobile-xs:grid-cols-1 grid-cols-1 mobile-l:grid-cols-2 sm:grid-cols-2 tablet:grid-cols-3 lg:grid-cols-3 gap-3 mobile-l:gap-4 sm:gap-5 tablet:gap-6 lg:gap-8">
          {flashDeals.map((product) => (
            <Link key={product.id} to={`/flash-deals/${product.id}`} className="group">
              <Card className="overflow-hidden hover:shadow-2xl transition-all duration-300 border-0 shadow-lg hover:scale-105 transform flex flex-col mobile-xs:h-[380px] h-[400px] mobile-l:h-[420px] sm:h-[440px] tablet:h-[460px]">
                <div className="relative flex-shrink-0">
                  ...
                  <div className="absolute top-2 left-2 mobile-l:top-2 mobile-l:left-2 sm:top-4 sm:left-4 bg-primary text-white px-1.5 py-0.5 mobile-l:px-2 mobile-l:py-1 sm:px-3 sm:py-1 rounded-md mobile-l:rounded-md sm:rounded-lg text-xs mobile-l:text-xs sm:text-sm font-bold shadow-lg">
                ...
                <CardContent className="p-2.5 mobile-xs:p-3 mobile-l:p-3 sm:p-4 tablet:p-5 lg:p-6 flex flex-col flex-1">
                  ...
                  <div className="flex items-center gap-1.5 mobile-l:gap-2 sm:gap-3 tablet:gap-3 mb-2 mobile-l:mb-3 sm:mb-4 flex-shrink-0">
                  ...
                  <div className="text-xs mobile-l:text-xs sm:text-sm tablet:text-base text-dark-gray mb-2 mobile-l:mb-3 sm:mb-4 flex-shrink-0">
                    {product.flashDealData?.sold || 0} sold • {formattedTime} left
                  </div>
                  <div className="w-full bg-medium-gray/30 rounded-full h-2 mobile-l:h-2 sm:h-3 mt-auto">
                    <div
                      className="bg-primary h-2 mobile-l:h-2 sm:h-3 rounded-full transition-all duration-300"
